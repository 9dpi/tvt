...
const App = {
  ...
  async init() {
    ...
    // Auto-detect if key is in clipboard (UX Magic)
    window.addEventListener('focus', () => this.checkClipboardForKey());
  },

  async checkClipboardForKey() {
    // Only if currently in offline mode and user gave permission before or we want to be subtle
    if (this.provider?.provider !== 'offline') return;
    try {
      // Note: This usually requires user interaction or permission, but we'll try
...
_Sau khi lưu, NIKOLA sẽ tự động nhận diện và bạn có thể bắt đầu tư duy ngay lập tức!_`, false);
  ...
  handleSend() {
    ...
    // Secret: Auto-detect API Keys in chat to help users
    if (text.startsWith('gsk_') || text.startsWith('AIza')) {
      const type = text.startsWith('gsk_') ? 'Groq' : 'Gemini';
      this.tvtSay(`✨ **NIKOLA đã nhận diện thấy mã ${type} của bạn!** Tôi đang lưu lại và kích hoạt...`, false);
      const conf = {};
      if (type === 'Groq') conf.groqKey = text;
      else conf.geminiKey = text;
      AI_PROVIDERS.saveConfig(conf);
      document.getElementById('msg-input').value = '';
      setTimeout(() => {
        this.detectAI();
        this.tvtSay(`✅ Đã cài đặt xong mã **${type}**. Bạn có thể tiếp tục phân tích rồi đấy!`);
      }, 1000);
      return;
    }
    ...
    if (!textVal || input.disabled) return;

    const mode = this._inputMode;
    const wc = TVTCore.countWords(textVal);
